...
void loop() {
  while(true) {
    ...
    if(Mag_acc>20 or hasLaunched==true) {
    PID(); //time to do some control
    }
    else launch_time = micros();
    
    //if statement to stop the loop after 30 minutes
    if(micros() > 1E7) { //1E9 is 30 mins 3E7 is 30 seconds 5E6 is 5 seconds
       break;
    }
    ...
    if(record_time > 5E6) { //1E9 is 30 mins 3E7 is 30 seconds 5E6 is 5 seconds
